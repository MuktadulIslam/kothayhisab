...
class _InventoryPageState extends State<InventoryPage> {
  ...
  List<InventoryEntry> filteredEntries = [];
  String errorMessage = '';
  ...
  void filterEntries(String query) {
    ...
    setState(() {
      if (trimmedQuery.isEmpty) {
        ...
        filteredEntries = List.from(inventoryResponse!.items);
      } else {
        // Create a new list for filtered entries
        List<InventoryEntry> newFilteredEntries = [];

        for (var entry in inventoryResponse!.items) {
          // Find only the inventory items that match the search query
          List<InventoryItem> matchingItems =
              entry.inventoryDetails
                  .where(
          ...
          if (matchingItems.isNotEmpty) {
            ...
            InventoryEntry filteredEntry = InventoryEntry(
              inventoryText: entry.inventoryText,
              ...
              inventoryDetails: matchingItems,
            );
  ...
  Map<String, List<InventoryEntry>> _groupEntriesByDate(
    List<InventoryEntry> entries,
  ) {
    Map<String, List<InventoryEntry>> result = {};
  ...
  void _showDetailsBottomSheet(BuildContext context, InventoryEntry entry) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(entry.inventoryText),
                    ),
                    const SizedBox(height: 16),
                    ...
                    Container(
                      ...
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: List.generate(entry.inventoryDetails.length, (
                          index,
                        ) {
                          final item = entry.inventoryDetails[index];
                          final bool isFirstItem = index == 0;
                          final bool isLastItem =
                              index == entry.inventoryDetails.length - 1;
  ...
  Widget build(BuildContext context) {
    ...
    Map<String, List<InventoryEntry>> groupedEntries =
                        ...
                        itemBuilder: (context, dateIndex) {
                          ...
                          return Column(
                            ...
                            children: [
                              ...
                              // Display all inventory items for each date (flattened view but with group spacing and borders)
                              ...dateEntries.asMap().entries.expand((
                                entryWithIndex,
                              ) {
                                int entryIndex = entryWithIndex.key;
                                InventoryEntry entry = entryWithIndex.value;
                                ...
                                itemWidgets.add(
                                  Container(
                                    ...
                                    child: Column(
                                      children:
                                          entry.inventoryDetails.asMap().entries.map((
                                            itemEntry,
                                          ) {
